...
def emojified(userguess: str, secretguess: str) -> str:
    ...
    while index < len(secretguess):
        ...
        elif contains_char(secretguess, userguess[index]) == True:
            result_emoji += YELLOW_BOX
        elif contains_char(secretguess, userguess[index]) == False:
            result_emoji += WHITE_BOX
        index += 1
    return result_emoji

def input_guess(lenguess) -> int:
    """Evaluates whether the length of a guess matches the length of the secret word"""
    user_guess: str = input(f"Enter a {lenguess} character word: ")
    while lenguess != len(user_guess):
        user_guess = input(f"That wasn't {lenguess} chars! Try again: ")
    return user_guess
